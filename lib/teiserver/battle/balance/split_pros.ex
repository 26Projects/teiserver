defmodule Teiserver.Battle.Balance.SplitPros do
  @moduledoc """
  SplitPros Kill-Switch balancer.
  ...
  Scoring: score = diff + λ * hoarding_penalty

  Tuning:
  - Increase λ (@lambda) to punish hoarding more strongly.
  - Increase @bin_margin if you want to allow anchors’ teams to hoard more.
  - Raise @max_iters if you want longer search (better results, slower).
  """
  ...
  def perform(expanded_group, team_count, _opts \\ []) do
    ...
    # Kill switch: flatten players (ignore parties/avoids)
    players =
      for %{
            members: members,
            ratings: ratings,
  ...
  defp seed(players) do
    ...
    logs = [
      "Seed anchors: #{anchor1.name}(#{anchor1.rating}) | #{anchor2.name}(#{anchor2.rating})",
      "Closest to avg: above=#{inspect(above)} below=#{inspect(below)}",
      "Initial diff: #{diff(a1s, b1s)}"
    ]
  ...
  defp improve_until_local_optimum(players, a, b, max_iters) do
    ...
    [top_anchor | _] = Enum.sort_by(players, & &1.rating, :desc)
  ...
  defp do_improve(_players, a, b, _top_anchor, _cb, current, iters, max_iters, logs)
       ...
       do: {a, b, logs ++ ["Stop: iteration cap (#{iters}/#{max_iters}), score=#{current}"]}
  ...
  defp do_improve(players, a, b, top_anchor, cb, current, iters, max_iters, logs) do
    ...
    cond do
      best_pair == nil or best_delta <= 0.0 ->
        {a, b, logs ++ ["Stop: local optimum; score=#{current}"]}

      true ->
        {a2, b2, s2} = elem(best_pair, 1)
        do_improve(players, a2, b2, top_anchor, cb, s2, iters + 1, max_iters,
          logs ++ ["Swap improves score → #{current} → #{s2} (iter #{iters + 1})"]
        )
    end
  end
  ...
  defp summarize(a, b) do
    ...
    ["Final diff=#{diff(a,b)}, common_bin=#{cb}, " <>
